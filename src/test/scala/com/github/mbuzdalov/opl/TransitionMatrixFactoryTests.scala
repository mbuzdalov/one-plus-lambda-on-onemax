...
class TransitionMatrixFactoryTests extends AnyFlatSpec with Matchers:
  ...
  for d <- 1 to 100 do they should s"produce the same results for n=100, d=$d" in validate(100, d)
  ...
  for (n, d) <- Seq(
    ...
    (10000, 9948),
    (10000, 9961),
  ) do they should s"produce the same results for n=$n, d=$d" in validate(n, d)
