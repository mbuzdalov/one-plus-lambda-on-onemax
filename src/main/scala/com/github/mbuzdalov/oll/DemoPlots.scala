...
object DemoPlots:
  private def varyingLastTwoLambdas(): Unit =
    ...
    val tasks = java.util.ArrayList[Callable[Result]]()
    val lambdaCount = 100
    for (lambda1Idx <- 0 until lambdaCount; lambda2Idx <- 0 until lambdaCount) do
      tasks.add: () =>
    ...
    for Result(_, lambda1Idx, _, lambda2Idx, result) <- results do
  ...
  private def varyingLastLambda(): Unit =
    ...
    Using.resource(new PrintWriter("oll-binning-n500-last-lambda-v1.csv")): out =>
      ...
      for Result(popSize, lambda, result) <- results do
        if popSize == -1 
        then out.println(s"$lambda,smoothing,$result")
        else out.println(s"$lambda,$popSize,$result")

        if math.round(lambda) == popSize 
        then out.println(s"$lambda,rounding,$result")
    ...
    Using.resource(new PrintWriter("oll-binning-n500-last-lambda-v2.csv")): out =>
      ...
      for lastLambda <- allLastLambdas do
