...
object BestDynamicDoubleLambdaCheating:
  class Evaluator(ollComputation: OLLComputation, nonCachingComputation: OLLComputation, output: Option[String]):
    ...
    val totalRuntime: Double =
      ...
      val pool, pool2 = ScheduledThreadPoolExecutor(Runtime.getRuntime.availableProcessors())
      ...
      loopFromDownTo(n - 1, 0): x =>
        ...
        if testResults(4)._2 > testResults(5)._2 || testResults(5)._2 > testResults(6)._2 || testResults(6)._2 > testResults(7)._2 then
          // Find a large lambda that is better
          var prevValue = testResults(7)._2
          var popSize = n - 1
          var (currLambda, currValue) = schedulePopSize(popSize).get()
          while popSize > 1 && prevValue > currValue do
            if bestValue > currValue then
              bestValue = currValue
              bestLambda = currLambda
            popSize -= 1
