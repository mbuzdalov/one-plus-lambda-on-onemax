...
object OptimalStaticMutationRates:
  ...
  private def singular(n: Int, k: Int): Array[Double] = Array.tabulate(n + 1)(i => if k == i then 1.0 else 0.0)

  private def rlsWithFixedNumberOfFlips(): Unit =
    Using.resource(PrintWriter("rls-fixed-flips.csv")): out =>
      val n = 1000
      val flips = Seq(1, 3, 5)
      val distributions = flips.map(f => f -> FixedNonNormalizedDistribution(Array.tabulate(n + 1)(i => if (i == f) 1.0 else 0.0))).toMap
      out.println(flips.mkString("k,", ",", ""))
      val listeners = for k <- 500 to 1000; (f, d) <- distributions yield 
        (k, f, OptimalRunningTime(n - k).newListener(d))
      OnePlusLambda.apply(n, 1, listeners.map(_._3), printTimings = false)
      for (k, listenersK) <- listeners.groupBy(_._1).toIndexedSeq.sortBy(_._1) do
        out.println(listenersK.sortBy(_._2).map(_._3.toResult.expectedRunningTime + 1).mkString(s"$k,", ",", ""))

  private def targetStaticRLS(): Unit = 
    val n = 1000
    Using.resource(PrintWriter("rls-target-static.csv")): out =>
      out.println("k,f,v")
      for k <- 1000 to 500 by -1 do
        val listeners = (1 to 1000).map(b => OptimalRunningTime(n - k).newListener(FixedNonNormalizedDistribution(singular(n, b))))
  ...
  private def optimalDynamicRLS(): Unit = 
    val n = 1000
    Using.resource(PrintWriter("rls-optimal-dynamic.csv")): out =>
    ...
    Using.resource(PrintWriter("rls-target-dynamic.csv")): out =>
      ...
      for k <- 500 to 700 do
        ...
        for r <- results do
          ...
          if v > 0 then out.print(r.optimalParameter(n - k))
        out.println()
  ...
  private def pictureTargetDynamicRLS(): Unit =
    ...
    val hh = vMax - 500 + 1
    val heatmap = BufferedImage(vMax - 500, hh, BufferedImage.TYPE_INT_RGB)
    for x <- 500 until vMax do
      val maxFlips = results.last.optimalParameter(n - x)
      for y <- 500 to vMax do
        val hx = x - 500
        val hy = hh - 1 - (y - 500)
  ...
  private def eaWithStaticParameters(): Unit =
    ...
    class Task(val target: Int):
      ...
      private var midLeftListener, midRightListener, midListener: Option[ComputationListener[Unit]] = None
      ...
      private def createListener(p: Double): ComputationListener[Unit] =
        OptimalRunningTime(n - target).newListener(FixedNonNormalizedDistribution(standard(n, p)))

      def nextListeners(isFinalRun: Boolean): Seq[BareComputationListener] =
        if midLeftListener.isDefined && midRightListener.isDefined then
    ...
    Using.resource(PrintWriter("1p1-optimal-static-rates.csv")): out =>
      ...
      for ((k, lp, lr), gr) <- finalResults.lazyZip(gvTasks) do
