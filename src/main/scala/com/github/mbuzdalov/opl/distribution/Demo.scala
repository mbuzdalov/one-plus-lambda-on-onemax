...
object Demo:
  private def make[@specialized P](dist: ParameterizedDistribution[P], n: Int, p: P): DoubleProbabilityVector =
    val pv = DoubleProbabilityVector(n)
    dist.initialize(n, p, pv)
    val sum = pv.sum
    pv.setBounds(0, pv.largestDistance)
    if math.abs(1 - sum) > 1e-10 then pv.setValue(0, 1 - sum)
    pv
  ...
  def main(args: Array[String]): Unit =
    ...
    for n <- Seq(3, 5, 8, 11, 16, 23, 32, 45, 64, 91, 100) do
      val vectors = generators.map(_._2.apply(n))
      for d <- 0 to n do println(vectors.map(_.probability(d)).mkString(s"$n,$d,", ",", ""))
