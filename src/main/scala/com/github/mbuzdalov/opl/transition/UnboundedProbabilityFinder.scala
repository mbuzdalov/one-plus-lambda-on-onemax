...
object UnboundedProbabilityFinder extends TransitionProbabilityFinder {
  override def find(n: Int, lambda: Int, d: Int, change: Int, target: Array[Double], aux: Aux): Unit = {
    ...
    unit(0) = 1 - sum
    prob(0) = 1
    if (unit(0) < 1) ProbabilityVectorUtils.multiplyByPower(lambda, unit, prob, m)

    var i = u - lR
    while (i >= 0) {
      target(i) = prob(i + lR - l + 1)
      i -= 1
    }
  }
  ...
  override def newAuxiliaryData(n: Int): AuxImpl = AuxImpl(new Array(n + 1), new Array(n + 1))
}
